...
import { type Engagement, type Workstream, type DeliverableStatus } from "@/lib/mock-data";
...
function WorkstreamRow({ ws, outOfScope, onToggleDeliverable, onToggleWorkstream }: WorkstreamRowProps) {
  ...
  return (
    <div className={`border rounded-lg overflow-hidden transition-opacity duration-200 ${
      ...
      >
        ...
        {/* Workstream scope toggle — appears on header hover */}
        <AnimatePresence>
          {headerHovered && (
            <motion.button
              initial={{ opacity: 0 }}
              ...
              {ws.deliverables.map((d) => {
                ...
                return (
                  <li
                    ...
                    className={`group flex items-center gap-3 px-5 py-2.5 transition-opacity duration-150 ${
                      oos ? "opacity-40" : ""
                    }`}
                  >
                    {/* Status dot or OOS dash */}
                    {oos ? (
                      <span className="h-1.5 w-1.5 shrink-0 flex items-center justify-center text-muted text-[8px] leading-none">—</span>
                    ) : (
                      <div className={`h-1.5 w-1.5 rounded-full shrink-0 ${STATUS_DOT[d.status]}`} />
                    )}
                    ...
                    {/* Status label or OOS label */}
                    <span className={`shrink-0 text-[10px] font-medium font-mono transition-colors ${
                      oos ? "text-muted" : STATUS_TEXT[d.status]
                    }`}>
                      {oos ? "Out of Scope" : STATUS_LABEL[d.status]}
                    </span>

                    {/* Scope toggle — revealed on row hover */}
                    <button
                      onClick={() => onToggleDeliverable(d.deliverable_id)}
                      title={oos ? "Restore to scope" : "Remove from scope"}
                      className={`shrink-0 opacity-0 group-hover:opacity-100 transition-opacity ml-2 w-5 h-5 flex items-center justify-center rounded text-[11px] font-medium ${
                        oos
                          ? "text-accent hover:bg-accent/20"
                          : "text-muted hover:bg-error/15 hover:text-error"
                      }`}
                    >
                      {oos ? "+" : "−"}
                    </button>
...
export function WorkplanPanel({ engagement }: WorkplanPanelProps) {
  ...
  // Initialise from data model; local until backend persistence is wired
  ...
  const toggleWorkstream = (wsId: string, allOos: boolean) => {
    ...
    setOutOfScope((prev) => {
      ...
      if (allOos) {
        // restore all
        ws.deliverables.forEach((d) => next.delete(d.deliverable_id));
      } else {
        // mark all OOS
        ws.deliverables.forEach((d) => next.add(d.deliverable_id));
      }
      return next;
    });
  };
  ...
  // Progress counts exclude OOS deliverables
